...
// ✅ IMPORTANT: plant & validator remain in schema (so they are sent/available),
// but user will NOT choose them; we set them automatically.
        ...
        >
          <div className="space-y-6">
            <div className="animate-fade-in-up" style={{ animationDelay: "0.1s" }}>
              ...
              <input className={inputBase} placeholder="Identify the main cause..." value={local.root_cause} onChange={(e) => setLocal((p) => ({ ...p, root_cause: e.target.value }))} />
            </div>
            ...
            <div className="animate-fade-in-up" style={{ animationDelay: "0.15s" }}>
              ...
              <textarea
                rows={4}
                className={inputBase}
                placeholder="Describe the identified cause in detail..."
                value={local.detailed_cause_description}
                onChange={(e) => setLocal((p) => ({ ...p, detailed_cause_description: e.target.value }))}
              />
            ...
            <div className="animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
              ...
              <textarea
                ...
                value={local.solution_description}
                onChange={(e) => setLocal((p) => ({ ...p, solution_description: e.target.value }))}
              />
            ...
            <div className="animate-fade-in-up" style={{ animationDelay: "0.25s" }}>
              ...
              <textarea rows={4} className={inputBase} placeholder="Conclude on this root cause..." value={local.conclusion} onChange={(e) => setLocal((p) => ({ ...p, conclusion: e.target.value }))} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-in-up" style={{ animationDelay: "0.3s" }}>
              <SearchableSelect
                ...
                value={local.process}
                onChange={(v) => setLocal((p) => ({ ...p, process: v }))}
    ...
    >
      ...
      <div className="flex items-start justify-between pl-4">
        ...
        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all duration-200">
          ...
          <button type="button" onClick={onRemove} className="p-2 rounded-xl text-[#585858] hover:bg-red-50 hover:text-red-500 transition-all duration-200" title="Delete">
            <Icons.Trash className="w-4 h-4" />
          </button>
        </div>
      </div>
...
// ================ MAIN FORM ================
export default function LlcFormTailwind() {
  ...
  // ✅ inject plant & validator into form state (required by schema)
  useEffect(() => {
        ...
        <div className="rounded-3xl bg-white shadow-xl shadow-[#046eaf]/5 overflow-hidden animate-fade-in-up">
          <div className="relative px-8 py-8 bg-gradient-to-r from-[#046eaf] via-[#0e4e78] to-[#046eaf] bg-[length:200%_100%]">
            ...
            <div className="relative flex items-center gap-5">
              ...
              <div>
                <h1 className="text-3xl font-bold text-white">Quality Lesson Learned</h1>
                <p className="mt-1 text-m text-white/80">Document and share quality learnings</p>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <input className={inputBase} placeholder="Summarize the problem in a few words..." {...register("problem_short")} />
                </div>
                <div>
                  <Label required>Detailed Problem Description</Label>
                  <textarea rows={6} className={inputBase} placeholder="What? Why? When? Who? Where? How? How many? " {...register("problem_detail")} />
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <RHFSearchableSelect control={control} name="llc_type" rules={{ required: true }} label="LLC Type" required options={OPTIONS.llc_type} placeholder="Select llc type" />
                  <RHFSearchableSelect control={control} name="customer" rules={{ required: true }} label="Customer" required options={OPTIONS.customer} placeholder="Select customer" />
                  <div>
                    <Label required>Product Family</Label>
                    <input className={inputBase} {...register("product_family")} />
                  </div>
                  <RHFSearchableSelect control={control} name="product_type" rules={{ required: true }} label="Product Type" required options={OPTIONS.product_type} placeholder="Select product type" />
                  <RHFSearchableSelect control={control} name="quality_detection" rules={{ required: true }} label="Quality Detection" required options={OPTIONS.quality_detection} placeholder="Select quality detection" />
                  <RHFSearchableSelect control={control} name="application_label" rules={{ required: true }} label="Application Label" required options={OPTIONS.application_label} placeholder="Select application label" />
                  <RHFSearchableSelect control={control} name="product_line_label" rules={{ required: true }} label="Product Line Label" required options={OPTIONS.product_line_label} placeholder="Select product line" />
                  <div>
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <ReadOnlyField
                    ...
                    required
                    value={userEditor}
                    help={!userEditor ? "No editor found in localStorage.user. Please sign in again." : undefined}
                  />
                  <ReadOnlyField
                    label="Plant"
                    required
                    value={userPlant}
                    help={!userPlant ? "No plant found in localStorage.user. Please sign in again." : undefined}
                  />

                  <RHFSearchableSelect control={control} name="failure_mode" rules={{ required: true }} label="Failure Mode" required options={OPTIONS.failure_mode} placeholder="Select failure mode" />
                </div>
              </div>

              {/* Quality: Bad/Good parts */}
              {isQuality && (
                <div className="space-y-5 p-6 rounded-2xl bg-gradient-to-br from-[#046eaf]/5 to-transparent border border-[#046eaf]/20 animate-fade-in">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                      <Label required>Bad Part</Label>
                      <UploadList files={badPartFiles} setFiles={setBadPartFiles} />
                    </div>
                    <div>
                      <Label required>Good Part</Label>
              ...
              <div className="space-y-5">
                ...
                {fields.length === 0 ? (
                  <div className="text-center py-12 rounded-2xl border-2 border-dashed border-[#c5c5c4] bg-[#f8fafc]">
                    ...
                    <p className="mt-1 text-xs text-[#585858]">Click "Add" to get started</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {fields.map((f, idx) => (
                      <RootCauseCard
                        ...
                        fileCount={rootCauseFiles[idx]?.length || 0}
                        onRemove={() => {
                          remove(idx);
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <textarea rows={5} className={inputBase} placeholder="Summarize..." {...register("conclusions")} />
                  <Counter value={globalConc} />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    <Label required>Situation Before</Label>
                    <UploadList files={situationBeforeFiles} setFiles={setSituationBeforeFiles} />
                  </div>
                  <div>
                    <Label required>Situation After</Label>
              ...
              <div className="space-y-5">
                ...
                <ReadOnlyField
                  label="Validator"
                  required
                  value={computedValidator}
                  help={userPlant && !computedValidator ? `No validator configured for: ${userPlant}` : undefined}
                />
              </div>

              {/* Submit */}
              <div className="pt-6 border-t border-[#c5c5c4]/30 flex justify-center">
                <button
                  type="submit"
