...
export default function PmReviewPage() {
  ...
  async function load() {
    try {
      ...
      const res = await fetch(
        `${API}/llc/${llcId}/pm-review?token=${encodeURIComponent(token)}`
      );
  ...
  const decided = ["APPROVED", "REJECTED"].includes(data.pm_decision);
            ...
            {decided ? (
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                ...
                <div className="mt-2 text-sm text-slate-700">
                  <div>
                    <b>Decision at:</b> {fmt(data.pm_decision_at)}
                  </div>
                  <div>
                    <b>Validation date PM:</b> {fmt(data.pm_validation_date)}
                  </div>
